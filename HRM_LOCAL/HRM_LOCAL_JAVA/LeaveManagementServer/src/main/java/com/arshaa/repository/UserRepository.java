...
//query for approve count
     ...
     @Query ( value= "select sum(h.`number_of_days`) as total from employeeleaves h WHERE h.`leave_status` IN ('Approved','Pending') and h.`leave_orwfh` IN ('W') and h.`employee_id`=:employeeId GROUP BY MONTH(curdate())",nativeQuery=true)
